	...
	else
	{
		...
		//control string can be SETUP, PLAY, PAUSE, TEARDOWN
		{
			...
			else
			{
				...
				//parse the video name into more useable info
				
				char_count = 0;
				char_length = get_word_size_single_array(movie_string, 0, "/");
				char_count += char_length + 2;
				char_length = get_word_size_single_array(movie_string, 0, "/");
				char *movie_name = (char *)malloc(char_length);
				set_word_double_array(movie_string, movie_name, 0, char_count, char_length);
				char_count += char_length + 1;
				free(movie_string);
...
void send_frame(union sigval sv_data) {
	...
	if(data != NULL)
	{
		...
		cloud_server cloud = get_cloud_server(data->video, data->frame);
		data->frame += data->scale - 1;
